...
# (iex does not support param blocks)
# ─────────────────────────────────────────────
$PreferredModel    = if ($env:CLAUDE_PREFERRED_MODEL) { $env:CLAUDE_PREFERRED_MODEL } else { "" }
$UseCloudFallback  = ($env:CLAUDE_USE_CLOUD -eq "true")
...
function Show-FinalInstructions {
    ...
    if ($IsCloud) {
        ...
        Write-Host ""
        Write-Host "  NOTE: Ensure you are logged in to Ollama:" -ForegroundColor Yellow
    ...
    Write-Host "  3. On the login screen select option 2" -ForegroundColor White
    Write-Host "     (Anthropic Console account · API usage billing)" -ForegroundColor White
    Write-Host "  4. When asked for an API key, type:  ollama" -ForegroundColor White
    Write-Host ""
    Write-Host "  ─── USEFUL COMMANDS ────────────────────────────────" -ForegroundColor DarkGray
    Write-Host "  Switch model  :  claude --model <name>" -ForegroundColor White
    Write-Host "  List models   :  ollama list" -ForegroundColor White
    Write-Host "  Pull model    :  ollama pull <name>" -ForegroundColor White
    Write-Host ""
}
...
# ─── Set execution policy for iex runs ──────
...
# Leave 4 GB headroom for the OS
...
if (Test-CommandExists "ollama") {
    ...
    if (-not (Test-CommandExists "ollama")) {
        ...
        Write-Host "  Please open a new PowerShell window and run the setup again." -ForegroundColor Yellow
        exit 1
    }
...
if ($PreferredModel -ne "") {
    Write-Info "Preferred model: $PreferredModel"
...
if ($selectedModel -eq "" -or $UseCloudFallback) {
    Write-Host ""
    if ($UseCloudFallback) {
        Write-Info "Cloud fallback requested via CLAUDE_USE_CLOUD=true"
    } else {
        ...
        if ($answer -notmatch "^[Yy]$") {
            Write-Failure "Setup cancelled. Free up RAM or set CLAUDE_PREFERRED_MODEL to a smaller model."
            exit 1
        }
    }
    ...
    if ($selectedModel -eq "") {
        ...
        Write-Host "  • Set a specific small model:" -ForegroundColor White
        Write-Host "      `$env:CLAUDE_PREFERRED_MODEL = 'llama3.2:3b'" -ForegroundColor Cyan
        Write-Host "      iex (irm <setup-url>)" -ForegroundColor Cyan
